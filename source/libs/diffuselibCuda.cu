...
void DiffuseXRD::WriteFitData(char* FileName){
  ...
  for(int k = 0; k<m_NumFitDatLines; k++){
    ...
    OutputFile<<m_FitQr[k]<<"\t"<<m_FitData[k]<<"\t"<<m_FitErrData[k]<<"\t"<<Yk<<"\n";
    // OutputFile<<m_r[k]<<"\t"<<m_SummationTable[k]<<"\n";
  }
  OutputFile.close();
}
...
void DiffuseXRD::WriteFitData2(char* FileName){
  ...
  for(int k = 0; k<m_NumFitDatLines; k++){
    Yk = (InterpolateQrScan(m_FitQr[k]))/NormFactor;
    OutputFile<<m_FitQr[k]<<"\t"<<m_FitData2[k]<<"\t"<<m_FitErrData2[k]<<"\t"<<Yk<<"\n";
    // OutputFile<<m_r[k]<<"\t"<<m_SummationTable[k]<<"\n";
  }
  OutputFile.close();
}
        ...
        while (std::getline(ss, token, ' '))     // convert each word on the stream into an int
        {
            if(ColumnCount==0){
              ...
              if(ColumnCount%2==1){
                ...
                gsl_matrix_set(m_FitData_comb,floor(ColumnCount/2),k,(double)(token.c_str()));
              }
              else{
                std::cout<<token<<" error"<<std::endl;
                gsl_matrix_set(m_FitErrData_comb,floor(ColumnCount/2)-1,k,(double)(token.c_str()));
              }
            }
            ColumnCount++;
        }
