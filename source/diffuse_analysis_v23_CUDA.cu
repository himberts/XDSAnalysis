        ...
        gsl_vector * f)
{
  ...
  for(int k = 0;k<(data->size1);k++){
    ...
    for (size_t i = 0; i < n; i++)
    {
        ...
        gsl_vector_set(xiS1, i, gsl_vector_get(xiS1, i)+(Yi - gsl_matrix_get(data,k,i))/gsl_matrix_get(sigma,k,i)); //!!!!
    }
...
int main(int argc, char const *argv[]) {
  ...
  desc.add_options()
      ...
      ("w,w", po::value<double>()->default_value(0.004), "Gaussian Width of the Beam in inverse Angstrom")
 ...
 if (SysModus==e_pccf){
   ...
   // return 0;
    ...
    for(int k =0; k<Simulation.m_FitData_comb->size1 ; k++){
      ...
      Simulation.PostProcessHankel();
      sprintf(FileName,"%s_%d_fitted.fit",FileNameBase,k);
      BackupFile(FileName);
      Simulation.WriteFitData(FileName,k);
    }

    // qztemp = Simulation.GetQzStart2();
    // qztemp = gsl_vector_get (Simulation.m_FitData_qzVals, 1);
    // std::cout<<qztemp<<endl;
    // Simulation.ListSimParameters();
    // Simulation.PreProcessNSummation(qztemp);
    // Simulation.HankelTransformation(qztemp);
